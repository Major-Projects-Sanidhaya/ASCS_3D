...
class ScoutController(BaseController):
    ...
    and silence detection against the parent Node.
    """
    ...
    def send_uplink(self, packet: dict) -> None:
        """Log a ScoutPacket emission event."""
    ...
    def handle_silence(self, source: str, dt: float) -> None:
        """Enter LOITER mode when Node silence is detected."""
        self._agent.handle_command_timeout()

    def reset(self) -> None:
        """Reconstruct the Scout agent and clear the event log."""
    ...
    def sense_and_emit(self, all_scouts: List) -> None:
        """Build a ScoutPacket from sensor reads and emit to the Node."""
        self._agent.emit_packet(self._agent._node, all_scouts)
        self.log_event('PACKET_EMITTED', {})

    # ── Safety ──

    def trigger_safety_reflex(self) -> bool:
        """Apply obstacle separation reflex if ToF triggers. Returns True if applied."""
        obs_fwd, obs_min = self._agent.read_tof_obstacle()
        if obs_min < Scout.OBS_REFLEX_M:
            v_away = (-self._agent.heading * Scout.MAX_SPEED).tolist()

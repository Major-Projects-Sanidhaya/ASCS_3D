...
class SwarmController:
    ...
    def spawn_node(self, zone_hash: int) -> NodeController:
        ...
        # Scouts in a ring at radius 1.5 m
        for i in range(self._n_scouts):
            angle = 2 * math.pi * i / self._n_scouts
        ...
        # Workers in a ring at radius 2.0 m
    ...
    def despawn_node(self, zone_hash: int) -> None:
        ...
        TODO:
        1. Retrieve node = self._nodes.get(zone_hash); raise KeyError if absent.
        2. Remove each of node._scout_controllers from self._scouts.
        3. Remove each of node._worker_controllers from self._workers.
        4. del self._nodes[zone_hash].
        5. Log the despawn event.
        """
    ...
    def get_swarm_status(self) -> dict:
        """
        Return a top-level swarm health snapshot suitable for display or logging.
        """
        ...
        silent  = [
            zh for zh, z in world.items()
            if z.get('status') == 'SILENT'
        ]
        healths = (
            [z.get('health', 1.0) for z in world.values()]
            if world else [1.0]
        )
    ...
    def set_reynolds_weights(
        ...
        """
        Push GUI slider values to every NodeController and cache for
        subsequent spawned nodes.
        """
        self._gui_weights = {
            'w_sep':   w_sep,
            'w_align': w_align,
    ...
    def inject_goal(self, waypoint: np.ndarray, mode: str) -> None:
        ...
        TODO:
        1. self._general.inject_goal(waypoint, mode).
        """
        raise NotImplementedError

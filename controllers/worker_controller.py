...
class WorkerController(BaseController):
    ...
    safe-hover entry. Anonymous by design — agent_id uses Python object id
    and is never transmitted in any packet.
    """
    ...
    def send_uplink(self, packet: dict) -> None:
        """
        Log a WorkerPacket emission event (actual transmission via emit_packet).
        """
    ...
    def handle_silence(self, source: str, dt: float) -> None:
        """
        Enter safe hover when Node silence is detected by the caller.
        """
        self._agent.handle_command_timeout()

    def reset(self) -> None:
        """
        Reconstruct the Worker agent from its current position, Node ref,
        and payload type, then clear the event log.
        """
        self._agent = Worker(
            self._agent.pos,
            self._agent._node,
    ...
    def get_task_state(self) -> str:
        ...
        TODO:
        1. Return self._agent._task_state.
        """
    ...
    def get_payload_type(self) -> str:
        ...
        TODO:
        1. Return self._agent._payload_type.
        """
        raise NotImplementedError
    ...
    def trigger_proximity_reflex(self) -> bool:
        """
        Read the proximity sensor and inject a separation velocity command
        if an obstacle is within PROXIMITY_REFLEX_M.

        Returns True if a reflex was applied, False otherwise.

        TODO:
        1. reflex_vel = self._agent.handle_proximity_reflex().
        2. If reflex_vel is not None:
